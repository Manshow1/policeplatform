 var markerArray = [];
    function getPolicemanAlermPosition() {
      //alert("getStudentAlermPosition");
      if(false == onBtn()){
        return false;
      }
      var s = document.getElementById("policeiemi").value;
      ...
      $.ajax({
        type:"post",
        url:ival+"getPolicemanAlermPosition",
        data:{"policeiemi":s,"placeAiemi":e,"placeBiemi":i},
        dataType:"json",
        ...
        success:function(datas) {
          ...
          for(var n=(json.length-1); 0<=n; n--){
            ...
            var description = json[n].uploadTime+"->"+json[n].description;
                markerArray.push(new ObjMarker(json[n].alarmType, description, json[n].longitude, json[n].latitude, json[n].uploadTime));
            }
    ...
    LoaderMap("allmap1", 116.404, 39.915, markerArray);
    var int=self.setInterval("getPolecemanAlermPosition()",10*1000);
    function onBtn(){
      var policetIemi = document.getElementsByName("policeIemi")[0].value;
      var placeAIemi = document.getElementsByName("placeAIemi")[0].value;
      var placeBIemi = document.getElementsByName("placeBImei")[0].value;
      if (studentIemi.length != 15) {
        alert("学生设备号长度必须为15！");
        return false;
      }
