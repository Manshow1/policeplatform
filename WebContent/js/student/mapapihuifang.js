...
//对象
 function ObjMarker(lng, lat, locateType, uploadTime)    {
        ...
        function SearchMap(divContainer, lng, lat, markerArray) {
            var map = new BMap.Map("allmap2"); //创建地图实例
            ...
            var callback = function (data) {
               ...
               if (poes.length == markerArray.length) {
                    ...
                    for (var z = 0; z < poes.length; z++) {
                    	var content = "定位时间："+ markerArray[z].uploadTime + "<hr>" + "定位类型：" + markerArray[z].locateType +"<hr>"+ "经纬度：" + markerArray[z].lng+"/" +markerArray[z].lat +"<hr>";
